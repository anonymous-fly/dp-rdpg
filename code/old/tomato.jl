...
function densityf(X::Array{Float64,2},k::Int64=35)
    k=k+1;
    n=size(X,1)
    dim=size(X,2)
    X=float.(X')
    ...
    elseif dim==1
        ...
        for i=1:n
            push!(f,k*(k+1)/(2*n*(4/3)*pi*sum(dists[i].^dim)))
        end
    end
    return f
end
...
function SortByDensity(X::Array,f::Array)
    pair=[X f]
    dox = size(X,2)
    pair = sortslices(pair,rev=true,by=x->x[dox+1], dims=1)
...
function TomatoClustering(G::Array{Array{Int64,1},1},f::Array,tao::Number)
    n=length(f)
    g=zeros(n);
    v=[i for i in 1:n]
    pair = [f v G]
    pairs = sortslices(pair,rev=true,by=x->x[1], dims=1);
...
function data2clust(data::Array,graph=1,k1=4,k2=4,tao=0.01)
   f=densityf(data,k2)
   if graph==1
        return TomatoClustering(createGraph(data,k1),f,tao)
    elseif graph==2
        return TomatoClustering(createGraph2(data,k1),f,tao)
    else
...
function PlotClustering(X::Array,S,mark=nothing)
...
function tomatoPlot(X; graph=1,k1=100,k2=4,tao=0.01, plt=true)
    @time S=data2clust(X,graph,k1,k2,tao)
    
    nclusts = length(S)
    clusts = repeat([-1], size(X, 1))
        
    for j in 1:nclusts
        clusts[S[j][2]] .= j
    end
    
    println("Number of clusters = $(length(S))")

    if plt
        @time points,plt= PlotClustering(X,S);
        display(plt)
    end
